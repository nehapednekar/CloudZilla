...
WebServerSubnet1_Name="${Stack_Name}-csye6225-web-server-subnet-1"

WebServerSubnet2_Name="${Stack_Name}-csye6225-web-server-subnet-2"
...
aws cloudformation create-stack --stack-name $Stack_Name --template-body file://csye6225-cf-networking.json --parameters ParameterKey=VPCName,ParameterValue=$VPC_Name ParameterKey=InternetGatewayName,ParameterValue=$InternetGateway_Name ParameterKey=PublicRouteTableName,ParameterValue=$PublicRouteTable_Name ParameterKey=PrivateRouteTableName,ParameterValue=$PrivateRouteTable_Name ParameterKey=WebServerSubnet1Name,ParameterValue=$WebServerSubnet1_Name ParameterKey=WebServerSubnet2Name,ParameterValue=$WebServerSubnet2_Name ParameterKey=DBServerSubnetName,ParameterValue=$DBServerSubnet_Name
